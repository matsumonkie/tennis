...
Elm.Alias = Elm.Alias || {};
Elm.Alias.make = function (_elm) {
   "use strict";
   _elm.Alias = _elm.Alias || {};
   if (_elm.Alias.values) return _elm.Alias.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   return _elm.Alias.values = {_op: _op};
};
Elm.Route = Elm.Route || {};
Elm.Route.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var UserRoute = {ctor: "UserRoute"};
   var HomeRoute = {ctor: "HomeRoute"};
   return _elm.Route.values = {_op: _op,HomeRoute: HomeRoute,UserRoute: UserRoute};
};
...
Elm.Router.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var Follow = function (a) {    return {ctor: "Follow",_0: a};};
   var view = F2(function (address,model) {
      var _p0 = model;
      if (_p0.ctor === "UserRoute") {
            return A2($Html.div,
            _U.list([]),
            _U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,Follow($Route.UserRoute))]),_U.list([$Html.text("home ")]))
                    ,A2($Html.a,_U.list([A2($Html$Events.onClick,address,Follow($Route.HomeRoute))]),_U.list([$Html.text("you are on user")]))]));
         } else {
            return A2($Html.div,
            _U.list([]),
            _U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,Follow($Route.UserRoute))]),_U.list([$Html.text("you are on home ")]))
                    ,A2($Html.a,_U.list([A2($Html$Events.onClick,address,Follow($Route.HomeRoute))]),_U.list([$Html.text("user")]))]));
         }
   });
   var NoOp = {ctor: "NoOp"};
   var setHash = function (path) {    return $Effects.task(A2($Task.map,function (_p1) {    return NoOp;},$History.setHash("coucou")));};
   var update = F2(function (action,model) {
      var _p2 = action;
      if (_p2.ctor === "NoOp") {
            return A2($Alias._op["=>"],model,$Effects.none);
         } else {
            var _p3 = _p2._0;
            if (_p3.ctor === "HomeRoute") {
                  return A2($Alias._op["=>"],$Route.HomeRoute,setHash("home"));
               } else {
                  return A2($Alias._op["=>"],$Route.UserRoute,setHash("user"));
               }
         }
   });
   var init = function () {    var noOp = $Effects.task($Task.succeed(NoOp));return A2($Alias._op["=>"],$Route.HomeRoute,noOp);}();
   return _elm.Router.values = {_op: _op,init: init,NoOp: NoOp,Follow: Follow,setHash: setHash,update: update,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var changeRoute = function (route) {
      var _p0 = route;
      switch (_p0)
      {case "home": return $Router.Follow($Route.HomeRoute);
         case "user": return $Router.Follow($Route.UserRoute);
         default: return $Router.Follow($Route.UserRoute);}
   };
   var router = A2($Signal.map,changeRoute,$History.hash);
   var app = $StartApp.start({init: $Router.init,update: $Router.update,view: $Router.view,inputs: _U.list([router])});
   var main = app.html;
   return _elm.Main.values = {_op: _op,app: app,main: main,changeRoute: changeRoute,router: router};
};
